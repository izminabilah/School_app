        ...
        <div class="relative rounded-t-md bg-primary-light bg-[url('/assets/images/knowledge/pattern.png')] bg-contain bg-left-top bg-no-repeat px-5 py-10 dark:bg-black md:px-10">
            ...
            <div class="relative">
                <div class="flex flex-col items-center justify-center sm:-ms-32 sm:flex-row xl:-ms-60">
                    ...
                    <div class="mb-2 text-center text-xl font-bold dark:text-white md:text-3xl">Kehadiran Siswa</div>
                </div>
                <p class="mb-9 text-center text-base font-semibold">Cari Kelas yang diinginkan</p>
                <form action="{{ route('search-absent-go') }}" method="GET" class="mb-16">
                    <div class="relative mx-auto max-w-[580px]">
                        <input type="text" placeholder="Masukkan kelas" class="form-input py-3 ltr:pr-[100px] rtl:pl-[100px]" id="search-absent-go"  name="search-absent-go">
    ...
    <div class="panel" @if($search_results_available) @else hidden @endif id="tables-absent">
        <div class="panel">
            <div class="flex items-center justify-between flex-wrap mb-5 ">
                <h5 class="font-semibold text-lg dark:text-white-light">Daftar Kehadiran {{$note}}</h5>
                <div class="flex justify-between gap-2" style="padding-left: 170px">
                    @if($previousMonth)
                        <a href="{{ route('search-absent-go', ['month' => $previousMonth, 'search-absent-go' => $search]) }}" class="flex justify-center font-semibold p-2 rounded-full transition bg-white-light text-dark hover:text-white hover:bg-primary dark:text-white-light dark:bg-[#191e3a] dark:hover:bg-primary">
                    ...
                    @if($nextMonth)
                         <a href="{{ route('search-absent-go', ['month' => $nextMonth, 'search-absent-go' => $search]) }}" class="flex justify-center font-semibold p-2 rounded-full transition bg-white-light text-dark hover:text-white hover:bg-primary dark:text-white-light dark:bg-[#191e3a] dark:hover:bg-primary">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 rtl:rotate-180">
                ...
                <div class="fixed inset-0 bg-[black]/60 z-[999] overflow-y-auto hidden" id="form-add-absent">
                    <div class="flex items-center justify-center min-h-screen px-4">
                        <div class="panel border-0 p-0 rounded-lg overflow-hidden md:w-full w-[90%] my-8">
                            <button type="button" class="absolute top-4 ltr:right-4 rtl:left-4 text-white-dark hover:text-dark" onclick="location.href='/go/activity/absent'">
                            ...
                            <div class="p-5">
                                <form action="/go/activity/absent/add" method="POST">
                                    @csrf
                                    <div class="mb-5">
                                    ...
                                    <div class="flex justify-end items-center mt-8">
                                        <button type="button" class="btn btn-outline-danger"
                                                onclick="location.href='/go/activity/absent'">Cancel</button>
                        ...
                        <tr>
                            ...
                            <td class="border-2">
                                <div class="flex gap-4 items-center justify-center">
                                    <a type="button" class="btn btn-sm btn-outline-primary" href="{{route('edit-absent-go', ['id' => $student->id])}}">Edit</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
